...
class pid():
    ...
    def pid_func(self,error):
        self.integration += self.Ki * self.error
        output = self.Kp * self.error + self.Ki * self.integration + self.Kd * (self.error - self.last_error)
...
class Turtle3Control(pid):
    def __init__(self):
        ...
        self.current_pos_sub = rospy.Subscriber("/odmo",Odometry,self.current_point_callback,queue_size=10)
        self.goal_pos_sub = rospy.Subscriber('/target_point',PointStamped,self.goal_point_callback,queue_size=10)
        self.rate = rospy.Rate(200)

        self.angle_pid = pid(1,0,0)
    ...
    def updatecommandVelocity(self):
        ...
        if(abs(angle_error) < 1):
            ...
            cmd_vel.linear.x = self.linear_pid(distance)
        else:
            cmd_vel.linear.x = 0
